...
from graspy.embed.base import BaseEstimator
from .base import SupervisedLearningPipeline
from joblib import Parallel, delayed
import re
def Flat(l):
	return l.reshape(l.shape[0], -1)
class GeneralPipeline(SupervisedLearningPipeline):
	r"""
	...
	def __init__(self,
		...
		super(GeneralPipeline, self).__init__(steps=learning_method, memory=memory, verbose=verbose, plot_method=plot_method, kfold=kfold)
		self.embedding_method = embedding_method
		self.flat_method = flat_method
		self.re = re.compile(r"^[Ee][Mm][Pp][Tt][Yy]\w*$")
		if self.embedding_method is None:
			self.steps = self.steps
		elif isinstance(self.embedding_method, str) and bool(self.re.match(self.embedding_method)):
			self.steps = [('Flat', FunctionTransformer(self.flat_method, validate=False))] + self.steps
		elif isinstance(self.embedding_method[1], BaseEstimator):
			self.steps = [self.embedding_method, ('Flat', FunctionTransformer(self.flat_method, validate=False))] + self.steps
		else:
			raise ValueError('Type of embedding method is incorrect')
	...
	def get_steps_name(self):
		...
		"""
		stp = [self.steps[0][0], 'Flat']
		stp.extend([x[1].__class__.__name__  for x in self.steps[2:]])
		return stp
	...
	def cross_val_score(self, dataset, labels, groups=None, vote=False, jobs=1):
		...
		if groups is None:
			test_results = Parallel(n_jobs=jobs)(delayed(self.train_test)(dataset[train_index],dataset[test_index], labels[train_index], labels[test_index], vote=vote) for train_index, test_index in self.kfold.split(dataset, labels))
		else:
			test_results = Parallel(n_jobs=jobs)(delayed(self.train_test)(dataset[train_index],dataset[test_index], labels[train_index], labels[test_index], vote=vote, group_test=groups[test_index]) for train_index, test_index in self.kfold.split(dataset, labels, groups))
