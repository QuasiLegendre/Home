...
class MASEPipeline(SupervisedLearningPipeline):
	r"""
    Multiple Adjacency Spectral Embedding Pipeline (MASEPipeline) embeds arbitrary number of input
    graphs with matched vertex sets, then put matrix flattening and other sklearn transforms after
    the MASE, and finally defines a classifier.

    From graspy.embed.MASEClassifier
    ...
    After reaching the generated MASE :math:`R^{(i)}` matrices, a flattening function is applied to them.
    which transforms them to vectors that can be participated in the sklearn functions.
	...
	def __init__(self, 
		...
		super(MASEPipeline, self).__init__(steps=learning_method, memory=memory, verbose=verbose, plot_method=plot_method, kfold=kfold, flat_method=flat_method)
		if not isinstance(self.steps[0][1], MultipleASE):
			self.steps = [('MASE', MultipleASE()), ('Flat', FunctionTransformer(self.flat_method, validate=False))] + self.steps
