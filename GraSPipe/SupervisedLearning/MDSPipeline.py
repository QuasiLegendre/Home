...
class MDSPipeline(SupervisedLearningPipeline):
	r"""
	From graspy.embed.ClassicalMDS
	...
	def __init__(self, 
		...
		super(MDSPipeline, self).__init__(steps=learning_method, memory=memory, verbose=verbose, plot_method=plot_method, kfold=kfold)
		#self.LM = learning_method[0][1]
		self.flat_method = flat_method
		if not isinstance(self.steps[0][1], ClassicalMDS):
			self.steps = [('MDS', ClassicalMDS()), ('Flat', FunctionTransformer(self.flat_method, validate=False))] + self.steps
